...
use terminal::{Cursor, Iter, Row, Terminal};
...
enum Selection {
    Normal { start: (u32, u32), end: (u32, u32) },

    Block { start: (u32, u32), end: (u32, u32) },
...
impl Overlay {
    ...
    fn at_start(&self) -> bool {
        let (x, y) = overlay!(self; cursor);
        let back =
            self.inner.grid().back().len() as u32 + if self.status.is_some() { 1 } else { 0 };

        self.scroll == back && x == 0 && y == 0
    }
    ...
    pub fn key(&mut self, key: Key) -> (vec::IntoIter<Action>, touched::Iter) {
        ...
        let command = match *key.value() {
            Value::Char(ref ch) => match &**ch {
                ...
                ch if key.modifier().is_empty()
                    && self.hinter.hints.is_some()
                    && self.hinter.selected.is_none() =>
                {
                ...
                ch if prefix == Some(b'f') => Command::Move(command::Move::Next(
                    times.unwrap_or(1),
                    command::Next::Match(command::Match::After(ch.into())),
                )),

                ch if prefix == Some(b'F') => Command::Move(command::Move::Previous(
                    times.unwrap_or(1),
                    command::Previous::Match(command::Match::After(ch.into())),
                )),

                ch if prefix == Some(b't') => Command::Move(command::Move::Next(
                    times.unwrap_or(1),
                    command::Next::Match(command::Match::Before(ch.into())),
                )),

                ch if prefix == Some(b'T') => Command::Move(command::Move::Previous(
                    times.unwrap_or(1),
                    command::Previous::Match(command::Match::Before(ch.into())),
                )),

                // Keys for exits.
                "c" if key.modifier() == key::CTRL => Command::Exit,

                "i" if key.modifier().is_empty() => Command::Exit,

                "q" if key.modifier().is_empty() => Command::Exit,
                ...
                "^" | "0" => Command::Move(command::Move::Start),
                ...
                // Selection commands.
                "v" if key.modifier().is_empty() => Command::Select(command::Select::Normal),

                "v" if key.modifier() == key::CTRL => Command::Select(command::Select::Block),

                "V" if key.modifier() == key::SHIFT => Command::Select(command::Select::Line),

                "y" if key.modifier().is_empty() => Command::Copy(
                    match times {
                        Some(1) => Clipboard::Primary,
                        Some(2) => Clipboard::Secondary,
                        _ => Clipboard::default(),
                ...
                "p" if key.modifier().is_empty() => Command::Paste(
                    match times {
                        Some(1) => Clipboard::Primary,
                        Some(2) => Clipboard::Secondary,
                        _ => Clipboard::default(),
            ...
            Value::Button(button) => match button {
                // Exit commands.
                Button::Escape if key.modifier().is_empty() => Command::Exit,

                // Ignore keys while in hint mode.
                _ if self.hinter.hints.is_some() => Command::None,

                // Scrolling commands.
                Button::PageUp => Command::Scroll(command::Scroll::PageUp(times.unwrap_or(1))),

                Button::PageDown => Command::Scroll(command::Scroll::PageDown(times.unwrap_or(1))),

                // Cursor movement commands.
                Button::Left => Command::Move(command::Move::Left(times.unwrap_or(1))),

                Button::Down => Command::Move(command::Move::Down(times.unwrap_or(1))),

                Button::Up => Command::Move(command::Move::Up(times.unwrap_or(1))),

                Button::Right => Command::Move(command::Move::Right(times.unwrap_or(1))),

                Button::Home => Command::Move(command::Move::Start),

                Button::End => Command::Move(command::Move::End),
            ...
            Value::Keypad(button) => match button {
                ...
                // Cursor movement commands.
                Keypad::Home => Command::Move(command::Move::Start),

                Keypad::End => Command::Move(command::Move::End),

                Keypad::Left => Command::Move(command::Move::Left(times.unwrap_or(1))),

                Keypad::Down => Command::Move(command::Move::Down(times.unwrap_or(1))),

                Keypad::Up => Command::Move(command::Move::Up(times.unwrap_or(1))),

                Keypad::Right => Command::Move(command::Move::Right(times.unwrap_or(1))),
    ...
    fn handle(&mut self, command: Command) -> Vec<Action> {
        ...
        if after != before {
            ...
            if let Some(status) = self.status.as_mut() {
                let x = after.0 + 1;
                let y = self.inner.grid().back().len() as u32
                    + self.inner.grid().view().len() as u32
                    - after.1;
    ...
    fn command(&mut self, command: Command) -> Vec<Action> {
        ...
        match command {
            ...
            Command::Move(command::Move::Next(
                times,
                command::Next::Word(command::Word::Start(boundary)),
            )) => {
                for _ in 0..times {
            ...
            Command::Move(command::Move::Previous(
                times,
                command::Previous::Word(command::Word::Start(boundary)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Next(
                times,
                command::Next::Word(command::Word::End(boundary)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Previous(
                times,
                command::Previous::Word(command::Word::End(boundary)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Next(
                times,
                command::Next::Match(command::Match::After(ch)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Previous(
                times,
                command::Previous::Match(command::Match::After(ch)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Next(
                times,
                command::Next::Match(command::Match::Before(ch)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Move(command::Move::Previous(
                times,
                command::Previous::Match(command::Match::Before(ch)),
            )) => {
                for _ in 0..times {
                    let mut c = overlay!(self; cursor);
            ...
            Command::Hint(command::Hint::Start(id)) => {
                ...
                let top =
                    self.inner.rows() - 1 - if self.status.is_some() { 1 } else { 0 } + self.scroll;

                let config = self.inner.config().overlay().hinter(id).clone();
                let content = self.selection(&Selection::Line {
                    start: top,
                    end: bottom,
                });
            ...
            Command::Hint(command::Hint::Pick(code)) => {
                ...
                if self
                    ...
                    .unwrap()
                    .iter()
                    .any(|(name, _)| name.starts_with(&selected))
                {
            ...
            Command::Hint(command::Hint::Open) => {
                ...
                if let Some(hint) = self.hinter.get() {
                    actions.push(Action::Open(
                        self.hinter.config.opener().map(String::from),
                        hint.into(),
                    ));
                }
            }
    ...
    fn selection(&self, selection: &Selection) -> String {
        ...
        match *selection {
            ...
            Selection::Line { start, end } => self.selection(&Selection::Normal {
                start: (0, start),
    ...
    fn hint<T: AsRef<str>>(&mut self, (start, end): (usize, usize), content: T) {
        ...
        let hint = if let Some(hints) = self.hinter.hints.as_mut() {
            ...
            Some(
                hints
                    .put(
                        (position.0.unwrap(), position.1.unwrap_or((x, y))),
                        url.replace('\n', ""),
                    )
                    .clone(),
            )
        ...
        match what {
            ...
            Highlight::Hint(hint, level) => {
                ...
                for ch in hint.label.graphemes(true).skip(level) {
                    if flag {
                        self.view
                            .insert((x, y), Cell::occupied(ch.into(), self.hinter.label.clone()));
                    } else {
                        self.view.remove(&(x, y));
                    }
