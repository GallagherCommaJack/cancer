...
use crossbeam_channel::{bounded as sync_channel, unbounded as channel, Receiver, Sender};
use libc::{c_char, c_int, c_ushort, c_void, winsize};
use libc::{
    close, dup2, execvp, fork, getpwuid, getuid, ioctl, openpty, read, setsid, signal, write,
};
use libc::{fcntl, F_GETFL, F_SETFL, O_NONBLOCK};
use libc::{SIGALRM, SIGCHLD, SIGHUP, SIGINT, SIGQUIT, SIGTERM, SIG_DFL, TIOCSCTTY, TIOCSWINSZ};
...
impl Tty {
    pub fn spawn(
        ...
        unsafe {
            ...
            match fork() {
                // Fork failed.
                -1 => Err(Error::Message("failed to fork".into())),
