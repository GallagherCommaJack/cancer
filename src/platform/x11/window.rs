...
use config::Config;
use crossbeam_channel::{bounded as sync_channel, select, unbounded as channel, Receiver, Sender};
use error;
use font::Font;
use picto::Region;
use platform::key;
...
impl Window {
    ...
    pub fn new(name: Option<&str>, config: Arc<Config>, font: Arc<Font>) -> error::Result<Self> {
        ...
        let (window, depth) = {
            ...
            for allowed in screen.allowed_depths().filter(|a| a.depth() == 32) {
                for item in allowed.visuals() {
                    depth = 32;
                    visual = item.visual_id();
            ...
            icccm::set_wm_size_hints(
                ...
                &icccm::SizeHints::empty()
                    ...
                    .min_size(
                        (font.width() + (margin * 2)) as i32,
                        (font.height() + spacing + (margin * 2)) as i32,
                    )
    ...
    pub fn run(&mut self, manager: Sender<Event>) -> error::Result<()> {
        ...
        let CLIPBOARD = xcb::intern_atom(&self.connection, false, "CLIPBOARD")
            .get_reply()
            .unwrap()
            .atom();
        let UTF8_STRING = xcb::intern_atom(&self.connection, false, "UTF8_STRING")
            .get_reply()
            .unwrap()
            .atom();
        let STRING = xcb::ATOM_STRING;
        let TARGETS = xcb::intern_atom(&self.connection, false, "TARGETS")
            .get_reply()
            .unwrap()
            .atom();
        let SELECTION = xcb::intern_atom(&self.connection, false, "CANCER_CLIPBOARD")
            .get_reply()
            .unwrap()
            .atom();
