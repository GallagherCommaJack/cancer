...
use error;
use platform::key::{self, Button, Key, Keypad, Lock, Modifier};
...
impl Keyboard {
    ...
    pub fn new(connection: Arc<ewmh::Connection>, locale: Option<&str>) -> error::Result<Self> {
        ...
        // Select events.
        {
            let map = xcb::xkb::MAP_PART_KEY_TYPES
        ...
        let device = xkb::x11::device(connection.deref().deref())?;
        let keymap = xkb::x11::keymap(
            connection.deref().deref(),
            device,
            &context,
            Default::default(),
        )?;
        let state = xkb::x11::state(connection.deref().deref(), device, &keymap)?;
        ...
        let (table, compose) = {
            let locale = locale
                ...
                .or(env::var("LANG").ok())
                .unwrap_or("C".into());
            let mut table = if let Ok(table) =
                xkb::compose::Table::new(&context, &locale, Default::default())
            {
                table
            } else {
    ...
    pub fn handle(&mut self, event: &xcb::GenericEvent) {
        match event.response_type() - self.extension().first_event() {
            xcb::xkb::NEW_KEYBOARD_NOTIFY | xcb::xkb::MAP_NOTIFY => {
                self.keymap = xkb::x11::keymap(
                    self.connection.deref().deref(),
                    self.device,
                    &self.context,
                    Default::default(),
                )
                .unwrap();
                self.state =
                    xkb::x11::state(self.connection.deref().deref(), self.device, &self.keymap)
                        .unwrap();
            }
    ...
    pub fn key(&mut self, code: u8) -> Option<Key> {
        ...
        .iter()
        .fold(Modifier::empty(), |modifier, &(n, m)| {
            if self
                .state
                .mods()
                .active(n, xkb::state::component::MODS_EFFECTIVE)
            {
                modifier | m
            } else {
                modifier
        ...
        Some(Key::new(
            match symbol {
                xkb::key::Tab | xkb::key::ISO_Left_Tab => Button::Tab.into(),

                xkb::key::Escape => Button::Escape.into(),

                xkb::key::Return => Button::Enter.into(),

                xkb::key::BackSpace => Button::Backspace.into(),

                xkb::key::Delete => Button::Delete.into(),

                xkb::key::Insert => Button::Insert.into(),

                xkb::key::Home => Button::Home.into(),

                xkb::key::End => Button::End.into(),

                xkb::key::Page_Up => Button::PageUp.into(),

                xkb::key::Page_Down => Button::PageDown.into(),

                xkb::key::Up => Button::Up.into(),

                xkb::key::Down => Button::Down.into(),

                xkb::key::Right => Button::Right.into(),

                xkb::key::Left => Button::Left.into(),
                ...
                xkb::key::KP_Enter => Keypad::Enter.into(),

                xkb::key::KP_Home => Keypad::Home.into(),

                xkb::key::KP_Begin => Keypad::Begin.into(),

                xkb::key::KP_End => Keypad::End.into(),

                xkb::key::KP_Insert => Keypad::Insert.into(),

                xkb::key::KP_Multiply => Keypad::Multiply.into(),

                xkb::key::KP_Add => Keypad::Add.into(),

                xkb::key::KP_Subtract => Keypad::Subtract.into(),

                xkb::key::KP_Divide => Keypad::Divide.into(),

                xkb::key::KP_Decimal => Keypad::Decimal.into(),

                xkb::key::KP_Page_Up => Keypad::PageUp.into(),

                xkb::key::KP_Page_Down => Keypad::PageDown.into(),

                xkb::key::KP_Up => Keypad::Up.into(),

                xkb::key::KP_Down => Keypad::Down.into(),

                xkb::key::KP_Right => Keypad::Right.into(),

                xkb::key::KP_Left => Keypad::Left.into(),
