...
impl platform::Proxy for Proxy {
    fn dimensions(&self) -> (u32, u32) {
        let reply = xcb::get_geometry(&self.connection, self.window)
            .get_reply()
            .unwrap();
    ...
    fn surface(&self) -> error::Result<cairo::Surface> {
        ...
        Err(error::platform::x11::Error::MissingDepth(24).into())
    }
    ...
    fn open(&self, through: Option<&str>, value: &str) -> error::Result<()> {
        Command::new(through.unwrap_or("xdg-open"))
            .arg(value)
            .spawn()?;
