...
use control::{self, Control, C0, C1, CSI, DEC, SGR};
use error;
...
use terminal::{cell, Cell, Grid, Iter, Sixel, Tabs, Touched};
...
impl Terminal {
    ...
    pub fn blinking(&mut self, value: bool) -> touched::Iter {
        ...
        for (x, y) in self.region.absolute() {
            match self.grid[(x, y)] {
                Cell::Empty { ref style, .. } | Cell::Occupied { ref style, .. }
    ...
    pub fn focus<O: Write>(&mut self, value: bool, mut output: O) -> io::Result<()> {
        if self.mode.contains(mode::FOCUS) {
            try!(output.write_all(if value { b"\x1B[I" } else { b"\x1B[O" }));
        }
    ...
    pub fn key<O: Write>(&mut self, key: Key, mut output: O) -> io::Result<()> {
        use platform::key::{Button, Keypad, Value};
        ...
        match *key.value() {
            ...
            Value::Button(Button::Enter) | Value::Keypad(Keypad::Enter) => write! {
                ALT # CRLF => b"\x1B\r\n",
                ALT        => b"\x1B\r",
            ...
            Value::Button(Button::Insert) | Value::Keypad(Keypad::Insert) => write! {
            ...
            Value::Button(Button::Home) | Value::Keypad(Keypad::Home) => write! {
            ...
            Value::Button(Button::End) | Value::Keypad(Keypad::End) => write! {
            ...
            Value::Button(Button::PageUp) | Value::Keypad(Keypad::PageUp) => write! {
                CTRL  => b"\x1B[5;5~",
                SHIFT => b"\x1B[5;2~",
                _     => b"\x1B[5~",
            },

            Value::Button(Button::PageDown) | Value::Keypad(Keypad::PageDown) => write! {
                CTRL  => b"\x1B[6;5~",
                SHIFT => b"\x1B[6;2~",
                _     => b"\x1B[6~",
            },

            Value::Button(Button::Up) | Value::Keypad(Keypad::Up) => write! {
                CTRL  => b"\x1B[1;5A",
                ALT   => b"\x1B[1;3A",
                SHIFT => b"\x1B[1;2A",
            ...
            Value::Button(Button::Down) | Value::Keypad(Keypad::Down) => write! {
                CTRL  => b"\x1B[1;5B",
                ALT   => b"\x1B[1;3B",
                SHIFT => b"\x1B[1;2B",
            ...
            Value::Button(Button::Right) | Value::Keypad(Keypad::Right) => write! {
                CTRL  => b"\x1B[1;5C",
                ALT   => b"\x1B[1;3C",
                SHIFT => b"\x1B[1;2C",
            ...
            Value::Button(Button::Left) | Value::Keypad(Keypad::Left) => write! {
                CTRL  => b"\x1B[1;5D",
                ALT   => b"\x1B[1;3D",
                SHIFT => b"\x1B[1;2D",
    ...
    pub fn input<I: AsRef<[u8]>, O: Write>(
        &mut self,
        input: I,
        mut output: O,
    ) -> error::Result<(vec::IntoIter<Action>, touched::Iter)> {
                ...
                match command {
                            ...
                            control::Result::Done(rest, header) => {
                                ...
                                let sixel = Sixel::new(
                                    ...
                                    self.cursor
                                        .style()
                                        .background()
                                        .unwrap_or(self.config.style().color().background()),
                    ...
                    Command::Sixel(mut sixel) => {
                            ...
                            control::Result::Done(rest, _) => {
                                ...
                                for (i, row) in sixel.into_inner().into_iter().enumerate() {
                                    ...
                                    for (offset, buffer) in row.into_iter().enumerate() {
                                        self.grid[(x + offset as u32, y)]
                                            .make_image(buffer, self.cursor.style().clone());
                        ...
                        match DEC::SIXEL::parse(input) {
                            ...
                            control::Result::Error(..) => input = &input[1..],

                            control::Result::Done(..) | control::Result::Incomplete(..) => {
                                debug!(target: "cancer::terminal::input", "incomplete input: {:?}", input);
                                self.cache = Some(input.to_vec());
                                self.command = Some(Command::Sixel(sixel));
                                break;
                    ...
                    Command::Internal => match C1::string(input) {
    ...
    fn control<O: Write>(&mut self, control: Control, mut output: O) -> error::Result<Vec<Action>> {
        ...
        match control {
            ...
            Control::C1(C1::ControlSequence(CSI::DeviceStatusReport(
                CSI::Report::CursorPosition,
            ))) => {
                try!(control::format_to(
                    output.by_ref(),
            ...
            Control::C1(C1::ControlSequence(CSI::Set(modes))) => {
                ...
                for mode in modes {
                    match mode {
                        CSI::Mode::KeyboardLock => self.mode.insert(mode::KEYBOARD_LOCK),

                        CSI::Mode::InsertionReplacement => self.mode.insert(mode::INSERT),

                        CSI::Mode::SendReceive => self.mode.insert(mode::ECHO),

                        CSI::Mode::LineFeed => self.mode.insert(mode::CRLF),
            ...
            Control::DEC(DEC::Set(modes)) => {
                ...
                for mode in modes {
                    match mode {
                        DEC::Mode::ApplicationCursor => self.mode.insert(mode::APPLICATION_CURSOR),
                        ...
                        DEC::Mode::AutoWrap => self.mode.insert(mode::WRAP),
                        ...
                        DEC::Mode::SmallFont => actions.push(Action::Resize(132, 24)),
            ...
            Control::C1(C1::ControlSequence(CSI::Private(b'h', None, args))) => {
                ...
                for arg in args.into_iter().flat_map(Option::into_iter) {
                    match arg {
                        1004 => self.mode.insert(mode::FOCUS),

                        2004 => self.mode.insert(mode::BRACKETED_PASTE),
                        ...
                        1006 => self.mode.insert(mode::MOUSE_SGR),

                        n => debug!(target: "cancer::terminal::unhandled", "unhandled set: {}", n),
                    }
                }
            }
            ...
            Control::C1(C1::ControlSequence(CSI::Reset(modes))) => {
                ...
                for mode in modes {
                    match mode {
                        CSI::Mode::KeyboardLock => self.mode.remove(mode::KEYBOARD_LOCK),

                        CSI::Mode::InsertionReplacement => self.mode.remove(mode::INSERT),

                        CSI::Mode::SendReceive => self.mode.remove(mode::ECHO),

                        CSI::Mode::LineFeed => self.mode.remove(mode::CRLF),
            ...
            Control::DEC(DEC::Reset(modes)) => {
                ...
                for mode in modes {
                    match mode {
                        DEC::Mode::ApplicationCursor => self.mode.remove(mode::APPLICATION_CURSOR),
                        ...
                        DEC::Mode::AutoWrap => self.mode.remove(mode::WRAP),
                        ...
                        DEC::Mode::SmallFont => actions.push(Action::Resize(80, 24)),
            ...
            Control::C1(C1::ControlSequence(CSI::Private(b'l', None, args))) => {
                ...
                for arg in args.into_iter().flat_map(Option::into_iter) {
                    match arg {
                        1004 => self.mode.remove(mode::FOCUS),

                        2004 => self.mode.remove(mode::BRACKETED_PASTE),

                        9 | 1000 | 1002 | 1003 => self.mode.remove(mode::MOUSE),

                        1006 => self.mode.remove(mode::MOUSE_SGR),
            ...
            Control::C1(C1::ControlSequence(CSI::SaveCursor)) | Control::DEC(DEC::SaveCursor) => {
            ...
            Control::C1(C1::ControlSequence(CSI::TabulationClear(
                CSI::Tabulation::AllCharacters,
            ))) => {
                self.tabs.clear();
            }
            ...
            Control::C1(C1::ControlSequence(CSI::SelectGraphicalRendition(attrs))) => {
                fn to_rgba(color: &SGR::Color) -> Rgba<f64> {
                    match *color {
                        SGR::Color::Transparent => Rgba::new(0.0, 0.0, 0.0, 0.0),

                        SGR::Color::Rgb(r, g, b) => Rgba::new_u8(r, g, b, 255),
                ...
                for mut attr in attrs {
                    if self.config.style().bold().is_bright() {
                        match attr {
                            ...
                            SGR::Reset | SGR::Foreground(_) => self.cursor.bright = None,
                    ...
                    match attr {
                        SGR::Reset => style = Style::default(),

                        SGR::Italic(true) => style.attributes.insert(style::ITALIC),
                        SGR::Italic(false) => style.attributes.remove(style::ITALIC),

                        SGR::Underline(true) => style.attributes.insert(style::UNDERLINE),
                        SGR::Underline(false) => style.attributes.remove(style::UNDERLINE),

                        SGR::Blink(true) => style.attributes.insert(style::BLINK),
                        SGR::Blink(false) => style.attributes.remove(style::BLINK),

                        SGR::Reverse(true) => style.attributes.insert(style::REVERSE),
                        SGR::Reverse(false) => style.attributes.remove(style::REVERSE),

                        SGR::Invisible(true) => style.attributes.insert(style::INVISIBLE),
                        SGR::Invisible(false) => style.attributes.remove(style::INVISIBLE),

                        SGR::Struck(true) => style.attributes.insert(style::STRUCK),
                        SGR::Struck(false) => style.attributes.remove(style::STRUCK),
                        ...
                        SGR::Foreground(ref color) => style.foreground = Some(to_rgba(color)),
                        ...
                        SGR::Background(ref color) => style.background = Some(to_rgba(color)),
                    }
                }
