...
impl Cell {
    ...
    pub fn is_default(&self) -> bool {
        if let Cell::Empty { ref style, .. } = *self {
            style.foreground.is_none() && style.background.is_none() && style.attributes.is_empty()
        } else {
            false
        }
    }
    ...
    pub fn is_wide(&self) -> bool {
        match *self {
            Cell::Empty { .. } | Cell::Image { .. } => false,

            Cell::Occupied { ref value, .. } => value.as_ref().width() > 1,

            Cell::Reference(..) => unreachable!(),
        }
    }

    /// Make the cell empty.
    pub fn make_empty(&mut self, style: Rc<Style>) {
        mem::replace(self, Cell::Empty { style: style });
    }
    ...
    pub fn set_style(&mut self, value: Rc<Style>) {
        match *self {
            ...
            | Cell::Occupied { ref mut style, .. } => *style = value,
    ...
    pub fn value(&self) -> &str {
        match *self {
            ...
            Cell::Occupied { ref value, .. } => value.as_ref(),

            Cell::Reference(..) | Cell::Image { .. } => "",
        }
    }
    ...
    pub fn width(&self) -> u32 {
        match *self {
            Cell::Empty { .. } | Cell::Image { .. } => 1,

            Cell::Occupied { ref value, .. } => value.as_ref().width() as u32,
    ...
    pub fn offset(&self) -> u32 {
        match *self {
            Cell::Reference(offset) => offset as u32,

            Cell::Empty { .. } | Cell::Occupied { .. } | Cell::Image { .. } => unreachable!(),
        }
    }
    ...
    pub fn image(&self) -> &cairo::Image {
        match *self {
            Cell::Image { ref buffer, .. } => buffer,

            Cell::Empty { .. } | Cell::Occupied { .. } | Cell::Reference(..) => unreachable!(),
        }
    }
}
