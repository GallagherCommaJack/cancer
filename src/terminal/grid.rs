...
impl Grid {
    ...
    pub fn resize(&mut self, cols: u32, rows: u32) -> i32 {
        ...
        fn resize(view: &mut VecDeque<Row>, free: &mut Free, cols: u32) -> i32 {
            ...
            for i in (0..view.len()).rev() {
                if view[i].is_wrapped() {
                    ...
                    let chunks = mem::replace(&mut wrapped, Vec::new())
                        .into_iter()
                        .rev()
                        .flat_map(|v| v.inner.into_iter())
        ...
        if self.view.len() > rows as usize {
            while self.view.len() > rows as usize
                && self.view.back().unwrap().iter().all(|v| v.is_empty())
            {
                self.view.pop_back();
            }
    ...
    pub fn up(&mut self, n: u32, region: Option<(u32, u32)>) {
        if let Some(region) = region {
            ...
            for i in 0..n {
                self.view
                    .insert(index + i as usize, self.free.pop(self.cols as usize));
            }
        } else {
