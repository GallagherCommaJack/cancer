    ...
    pub fn load(&mut self, table: &toml::value::Table) {
        ...
        if let Some(value) = table.get("bold").and_then(|v| v.as_str()) {
            match &*value.to_lowercase() {
                "bright" => self.bold = Bold::Bright,

                "normal" => self.bold = Bold::Normal,
        ...
        if let Some(value) = table.get("blink") {
            match *value {
                Value::Boolean(false) => self.blink = 0,

                Value::Integer(value) => self.blink = value as u32,
        ...
        if let Some(table) = table.get("cursor").and_then(|v| v.as_table()) {
            if let Some(value) = table.get("shape").and_then(|v| v.as_str()) {
                match &*value.to_lowercase() {
                    "block" => self.cursor.shape = Shape::Block,

                    "beam" | "ibeam" => self.cursor.shape = Shape::Beam,

                    "underline" | "line" => self.cursor.shape = Shape::Line,
