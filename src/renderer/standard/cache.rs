...
impl Cache {
    ...
    pub fn resize(&mut self, width: u32, height: u32) {
        ...
        let style = Rc::new(Style::default());
        self.inner
            .resize((width * height) as usize, Cell::empty(style.clone()));
    ...
    pub fn invalidate(&mut self, cell: &cell::Position) {
        ...
        self.inner[(cell.y() * self.width + cell.x()) as usize]
            .flags
            .remove(VALID);
    }
    ...
    pub fn update(&mut self, cell: &cell::Position, options: Options) -> bool {
        ...
        // Check if the cache is up to date, this is awful logic but alas.
        {
            ...
            if cache.flags.contains(VALID)
                ...
                && ((cell.is_empty() && cache.value.is_none())
                    || (cell.is_occupied()
                        && cache.value.as_ref().map(AsRef::as_ref) == Some(cell.value())))
            {
                return false;
            }
        }
        ...
        self.inner[index] = Cell {
            ...
            value: if cell.is_empty() {
                None
            } else {
                Some(cell.value().into())
            },
            flags: VALID
