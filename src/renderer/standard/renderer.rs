...
impl Renderer {
    ...
    pub fn margin(&mut self, state: &State, region: &Region) {
        ...
        let (c, f, o, s, h, v) = (
            ...
            state.font(),
            &mut self.context,
            state.config().style().spacing(),
            state.margin().horizontal,
            state.margin().vertical,
        );
    ...
    fn cursor(&mut self, state: &State, cursor: &cursor::Cell, options: Options) {
        ...
        o.save();
        {
            ...
            match cursor.shape() {
                ...
                Shape::Beam | Shape::Line => {
                    o.rgba(
                        cell.style()
                            .background()
                            .unwrap_or(c.style().color().background()),
                    );
                }
            }
            ...
            // Draw the glyph.
            if cell.is_occupied()
                && !(options.blinking() && cell.style().attributes().contains(style::BLINK))
            {
                ...
                match cursor.shape() {
                    ...
                    Shape::Beam | Shape::Line => {
                        o.rgba(
                            cell.style()
                                .foreground()
                                .unwrap_or(c.style().color().foreground()),
                        );
                    }
                }
                ...
                let computed = self
                    .glyphs
                    .compute(Rc::new(cell.value().into()), cell.style().attributes());
    ...
    fn cell(&mut self, state: &State, cell: &cell::Position, options: Options) -> bool {
        ...
        let mut fg = cell
            .style()
            .foreground()
            .unwrap_or_else(|| c.style().color().foreground());

        let mut bg = cell
            .style()
            .background()
            .unwrap_or_else(|| c.style().color().background());
